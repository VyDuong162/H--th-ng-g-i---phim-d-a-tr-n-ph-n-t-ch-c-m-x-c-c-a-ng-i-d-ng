...
// get id of the movie from the API (based on the name of the movie)
function get_id_movie_similar(arr,my_api_key){
  var list_movie_id =[];
  for(var m in arr) {
    $.ajax({
      type:'GET',
...
// get id of the movie from the API (based on the name of the movie)
function get_imdb_id_movie_similar(arr_id,my_api_key){
  var imdb_id_list =[];
  for(var m in arr_id) {

    $.ajax({
      type:'GET',
      url:'https://api.themoviedb.org/3/movie/'+arr_id[m]+'?api_key='+my_api_key,
      async: false,
      success: function(movie_details){
        imdb_id_list.push(movie_details.imdb_id);
      },
      error: function(){
        alert("API Error!");
        $("#loader").delay(500).fadeOut();
  ...
  return {imdb_id_list:imdb_id_list};
}